...
# Feito Por:
# GUILHERME DOMINGOS DOS SANTOS 21003431


def showOptions(students: list, menu='menu'):
    print()
...
def showStudentByRA(RA: str, students: list):
    studentExists = False
    for student in students:
        if student[0] == RA:
            studentExists = True
        ...
        try:
            ...
            while True:
                ...
                if studentRA == 'end':
                    ...
                    running = False
                    break
                elif studentRA != '' and studentRA.isdigit() == True:
                    for student in students:
...
def removeStudent(RA: str, students: list):
    studentExists = False
    for student in students:
        if student[0] == RA:
            studentExists = True
    ...
    if studentExists == True:
        showStudent(students[indexStudent])
        confirmacao = str(
            input('Deseja excluir os dados desse Aluno? (s/n): '))
        if confirmacao == 's':
...
def executeMenuOptions(option: str, students: list):
    if option == '1':
        ...
        if len(students) > 0:
            RA = str(input('Digite o RA do Aluno que seja excluir: '))
            removeStudent(RA, students)
        else:
            print('Não existe Alunos cadastrados')


def executeReportOptions(option: str, students: list):
    if option == '1':
        if len(students) > 0:
            RA = str(input('Digite o RA do Aluno que deseja visualizar: '))
            showStudentByRA(RA, students)
        else:
            print('Não existe Alunos cadastrados')
    elif option == '2':
        if len(students) > 0:
...
while True:
    try:
        showOptions(studentsList, menu)
        op = str(input('Insira o que deseja fazer: '))
        if op == '0':
            os.system('cls')
            print('\nPrograma Encerrado')
            break
        if op == '4' and menu == 'menu':
            menu = 'relatorio'
        os.system('cls')
        if menu == 'menu':
            executeMenuOptions(op, studentsList)

        if op == '3' and menu == 'relatorio':
            menu = 'menu'
        elif menu == 'relatorio':
            executeReportOptions(op, studentsList)
